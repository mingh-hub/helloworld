...
public class StatementDemoTest {
    ...
    private StatementDemo statementDemo;

    @Before
    public void setUp() {
        statementDemo = StatementDemo.builder()
    ...
    public void testForStringInject() throws Exception {
        ...
        List<SqlBean> result = statementDemo.queryWithStatement(connSql);
    ...
    public void testForStringJoint() throws Exception {
        ...
        List<SqlBean> result = statementDemo.queryWithStatement(sql);
        log.info("正常查询结果:{}", result);
        // 如果是接收前端入参, 用 Statement 极可能出现由 sql 拼接所导致的问题。java.sql.SQLSyntaxErrorException
        String userName = "Mr'Smith";
        String connSql = "select id, name, age, birthday, description, create_time, update_time from test where name='" + userName + "'";
        List<SqlBean> connResult = statementDemo.queryWithStatement(connSql);
